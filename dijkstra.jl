...
function lire_fichier1(nom::String)
...
function court_chemin1(depart, fin, carte, nom_graph)

   println("le point de depart est ", depart)
   
   println("le point d'arrivé est", fin)

    dx, dy = depart #un tuple contenant les coordonnées du point de départ sur la carte
    
    graph=lire_fichier1(carte) # on lit la carte
        ...
        for (x, y) in sens #pour chaque voisin du point extrait, on met a jour les distance si ce voisin est valide 
            ...
            if 1 <= nx <= taille && 1 <= ny <= taille && graph[nx,ny] != 0 #validité
                ...
                if new_poid < distance[nx, ny]
                    ...
                    push!(file_priorite, (nx, ny, new_poid))
                    sort!(file_priorite, by=x->x[3]) # Tri selon le poids
    ...
    while fin != depart # restitution du chemin trouvé
        ...
        for (x, y) in sens
            ...
            if 1 <= nx <= taille && 1 <= ny <= taille && distance[nx, ny] < distance[fin[1], fin[2]] #vérifie que les coordonnées du voisin (nx, ny) sont à l'intérieur des limites de la carte et que la distance de ce voisin est strictement inférieure à la distance du point actuel 
...
function tracer_graphe_chemin1(graph, chemin, nom_graph)
...
function tracer_court_chemin1(depart, fin, carte, nom_graph)

	(chemin,dist) = court_chemin1(depart, fin, carte, nom_graph) # on calcule le plus court chemin 
	...
	if chemin == [] || dist== typemax(Int) 
	   ...
	   tracer_graphe_chemin1(graph, chemin, nom_graph) # on trace le graph
