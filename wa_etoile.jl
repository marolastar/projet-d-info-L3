...
function lire_fichier3(nom::String)
...
function court_chemin3(depart, fin, carte,nom_carte,w)

    dx, dy = depart #un tuple contenant les coordonnées du point de départ sur la carte
    
    graph=lire_fichier3(carte) # on lit la carte
            ...
            if 1 <= nx <= taille && 1 <= ny <= taille && graph[nx,ny] != 0 #validité
                ...
                if new_poid < distance[nx, ny]
                   ...
                   distance[nx, ny] = new_poid 
                    ...
                    estimee_restante = w*(abs(nx - fx) + abs(ny - fy))
...
function tracer_graphe_chemin3(graph, chemin, nom_graph)
...
function tracer_court_chemin3(depart, fin, carte, nom_graph,w)

	(chemin,dist) = court_chemin3(depart, fin, carte, nom_graph,w) # on calcule le plus court chemin 
	...
	if chemin == [] || dist== typemax(Int) 
	   ...
	   tracer_graphe_chemin3(graph, chemin, nom_graph) # on trace le graph
...
function tps_exec(depart, fin, carte, nom_graph,w)
	...
	tracer_court_chemin3(depart, fin, carte, nom_graph,w)
